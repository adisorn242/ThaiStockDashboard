...
This repo is organized as dated snapshot folders (e.g. `20260912`). Each
folder is a complete, self-contained version of the dashboard — ready to
run or deploy as-is, with nothing outside that folder needed.

Each time the dashboard is meaningfully updated, a new dated folder is
added alongside the existing ones (named by date, e.g. `20261015`), with
the full, current set of files. Older dated folders are left untouched,
so any previous version can always be gone back to. The list below is
updated with a short summary each time a new version folder is added.
Every version folder also has its own `README.md` with this same
installation/usage information, in case that folder is used on its own.

- **`20260912`** (current / latest) — see "Features" below for what's
  included.

Everything from here down describes the current/latest version.

## Setup (easiest way — Windows, no typing commands)

1. **Double-click `Install.bat`** in the version folder (e.g. `20260912`).
   A window will open and set everything up automatically (this can take
   a few minutes the first time). When it says "Setup complete!", press
   any key to close it.
...
1. **Open the version folder in VS Code.**
   File → Open Folder → select the dated version folder (e.g. `20260912`).
